...
import { CREATIVE_WORK_MODES, MASTER_PERSONAS, SKILLS, buildMessages, getCreativeWorkMode, localFramework, parseSegments, routeRequest, type CompanionContext, type CreativeWorkMode, type OutputSegment } from "../lib/companion";
import { VoiceGuardian, type VoiceReference } from "./voice-guardian";

export type CompanionRun = { id: string; prompt: string; selected_skills: string[]; selected_persona: string; wisdom_enabled: boolean; output: { summary?: string; nextSteps?: string[]; segments?: OutputSegment[]; provenance?: string; workMode?: CreativeWorkMode }; provider: string; created_at: string };
...
export function AIStudioView({ user, notify, wisdomEnabled, context, runs, onRunSaved, onAppendToDraft }: {
  ...
  const [includeVoiceReferences, setIncludeVoiceReferences] = useState(false);
  const [voiceReferences, setVoiceReferences] = useState<VoiceReference[]>([]);
  const [workMode, setWorkMode] = useState<CreativeWorkMode>("build");
  ...
  const ask = async () => {
    ...
    const scoped: CompanionContext = { ...context, draftExcerpt: includeDraft ? context.draftExcerpt : "", sources: includeSources ? context.sources : [], voiceReferences: includeVoiceReferences ? voiceReferences.map(reference => ({ label: reference.label, excerpt: reference.excerpt })) : [] };
    ...
    try {
      const messages = buildMessages(clean, route, scoped, workMode);
      ...
      else output = localFramework(clean, route, scoped, workMode);
    } catch { output = localFramework(clean, route, scoped, workMode); }
    setSegments(output);
    ...
    if (supabase && user) {
      const { data } = await supabase.from("dreamboard_companion_runs").insert({
        ...
        output: { segments: output, workMode, provenance: "Working mode: " + getCreativeWorkMode(workMode).name + ". Human material: your prompt, draft, and sources. AI contribution: " + (provider === "connected-model" ? "generative review from your connected model, labeled by category." : "local deterministic routing only.") },
  ...
  return <section className="view ai-studio">
    ...
    <div className="ai-grid">
      <section className="ai-card">
        ...
        <fieldset className="work-mode-picker"><legend className="eyebrow">WORKING MODE · THIS CHANGES THE REVIEW LENS</legend><div className="companion-skills-row" role="radiogroup" aria-label="Creative Companion working mode">{CREATIVE_WORK_MODES.map(mode => <button key={mode.id} type="button" className={workMode === mode.id ? "season active" : "season"} onClick={() => setWorkMode(mode.id)} role="radio" aria-checked={workMode === mode.id} title={mode.description}>{mode.name}</button>)}</div><p className="assist-note"><b>{getCreativeWorkMode(workMode).name}:</b> {getCreativeWorkMode(workMode).boundary}</p></fieldset>
      ...
      <section className="ai-card ai-result">
        <span className="eyebrow">REVIEW PANEL · {getCreativeWorkMode(workMode).name.toUpperCase()}{routeInfo ? ` · ${routeInfo.split(".")[0]}` : ""}</span>
        <h3>{segments.length ? "A labeled review" : "Your Companion"}</h3>
        {segments.length ? <div className="segment-list">{segments.map((segment, index) => <div key={index} className={`segment segment-${segment.category}`}><b>{CATEGORY_LABELS[segment.category]}</b><p>{segment.text}</p></div>)}</div> : <p>Choose a working mode, then ask a question. Every answer is labeled by both its review lens and its evidence category. A private routing record is saved to your account when signed in.</p>}
    ...
    <VoiceGuardian user={user} projectId={context.projectId} projectTitle={context.projectTitle} onReferencesChange={setVoiceReferences} notify={notify} />
