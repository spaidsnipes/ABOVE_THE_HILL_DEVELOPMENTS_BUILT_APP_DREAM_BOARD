...
export default function Dreamboard() {
  ...
  const creativeGraph = useCreativeGraph(wmUser, setNotice);
  ...
  const addNote = (body: string, kind = "Imported") => {
    ...
    void (async () => {
      ...
      await supabase.from("dreamboard_graph_nodes").insert({ owner_id: wmUser.id, vault_entry_id: saved.id, node_type: "source", label: saved.title, description: saved.content.slice(0, 360) });
  ...
  return <main className={`os-shell theme-${dreamTheme}`}>
    ...
    <section className="stage">
      ...
      {active === "Creative Graph" && <CreativeGraphView graph={creativeGraph} signedIn={Boolean(wmUser)} onOpenSource={() => setActive("Knowledge Vault")} onVault={() => setActive("Knowledge Vault")} onPassport={() => setActive("Passport")} />}
