...
export default function Dreamboard() {
  ...
  return <main className={`os-shell theme-${dreamTheme}${focusMode ? " focus-mode" : ""}`}>
    ...
    <section className="stage">
      ...
      {active === "Writing Studio" && <WritingStudioView projectTitle={writingDocument?.title || "Untitled project"} chapterNumber={chapter + 1} chapterTitle={chapterTitle} draft={draft} setDraft={setDraft} saveStatus={saveStatus} signedIn={Boolean(passportUser)} onSaveVersion={saveSnapshot} onExport={exportDraft} onAskAI={() => setActive("AI Studio")} sources={notes.map(note => ({ id: note.id, title: note.title, body: note.body }))} focusMode={focusMode} setFocusMode={setFocusMode} wordCount={wordCount} />}
