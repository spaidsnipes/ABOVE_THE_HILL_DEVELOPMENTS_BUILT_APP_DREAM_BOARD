...
type ActiveView = "Creator’s Home" | "Creator Compass" | "Projects" | "Bulk Import" | "Knowledge Vault" | "Creative Graph" | "Book Architect" | "Writing Studio" | "Creative Timeline" | "Creation Journal" | "Version History" | "Reader" | "Audiobook Studio" | "AI Studio" | "Passport" | "Lounge" | "Shop" | "Radio" | "Settings";
...
type GraphNode = { id: string; node_type: string; label: string; description: string; vault_entry_id: string | null; created_at: string };
type GraphEdge = { id: string; from_node_id: string; to_node_id: string; relationship: string };
...
const chapters = ["Untitled chapter"];
const starterDraft = "";
const nav: Array<[string, ActiveView]> = [["⌂", "Creator’s Home"], ["◇", "Passport"], ["✧", "Creator Compass"], ["▦", "Projects"], ["⇧", "Bulk Import"], ["⌕", "Knowledge Vault"], ["⌬", "Creative Graph"], ["✦", "Book Architect"], ["✎", "Writing Studio"], ["◫", "Version History"], ["▤", "Reader"], ["◉", "Audiobook Studio"], ["◷", "Creative Timeline"], ["◫", "Creation Journal"], ["✦", "AI Studio"], ["◉", "Lounge"], ["▣", "Shop"], ["◌", "Radio"]];
...
export default function Dreamboard() {
  ...
  useEffect(() => {
    ...
    const loadWorkspace = async () => {
      const [vaultResult, nodesResult, edgesResult, companionResult, documentsResult] = await Promise.all([
        ...
        supabase.from("dreamboard_graph_nodes").select("id,node_type,label,description,vault_entry_id,created_at").order("created_at", { ascending: false }).limit(700),
        supabase.from("dreamboard_graph_edges").select("id,from_node_id,to_node_id,relationship").limit(1200),
      ...
      if (nodesResult.data) setGraphNodes(nodesResult.data as GraphNode[]);
      if (edgesResult.data) setGraphEdges(edgesResult.data as GraphEdge[]);
  ...
  const addNote = (body: string, kind = "Imported") => {
    ...
    void (async () => {
      ...
      const { data: entry, error } = await supabase.from("dreamboard_vault_entries").insert({ owner_id: wmUser.id, title, content: clean, source_type: sourceType, tags: ["Unsorted"] }).select("id,title,content,source_type,status,tags,created_at,updated_at").single();
      ...
      setVaultEntries(previous => [saved, ...previous]);
      const { data: node } = await supabase.from("dreamboard_graph_nodes").insert({ owner_id: wmUser.id, vault_entry_id: saved.id, node_type: "source", label: saved.title, description: saved.content.slice(0, 360) }).select("id,node_type,label,description,vault_entry_id,created_at").single();
      if (node) setGraphNodes(previous => [node as GraphNode, ...previous]);
      setNotice("Saved to your private Vision Vault and added to your Creative Graph.");
    })();
  };
  ...
  const uploadImportBatch = async () => {
    const supabase = getSupabaseBrowserClient();
    if (!supabase || !wmUser) { setNotice("Set up your Passport before starting a private import."); setActive("Passport"); return; }
  ...
  const saveSnapshot = () => { const words = draft.trim() ? draft.trim().split(/\s+/).length : 0; const snapshot: Snapshot = { id: Date.now(), label: `Chapter ${chapter + 1} · ${chapters[chapter]}`, body: draft, chapter, date: new Date().toLocaleString("en-US", { month: "short", day: "numeric", hour: "numeric", minute: "2-digit" }), words }; setSnapshots(prev => [snapshot, ...prev].slice(0, 20)); const supabase = getSupabaseBrowserClient(); if (!supabase || !wmUser) { setNotice("A protected local version was saved. Sign in with your Passport to add cloud history."); return; } void (async () => { let documentId = writingDocument?.id; if (!documentId) { const { data } = await supabase.from("dreamboard_writing_documents").insert({ owner_id: wmUser.id, title: `Untitled project — ${chapters[chapter]}`, chapter_number: chapter + 1, body: draft }).select("id,title,chapter_number,body,updated_at").single(); if (data) { const next = data as WritingDocument; setWritingDocument(next); documentId = next.id; } } else { await supabase.from("dreamboard_writing_documents").update({ title: writingDocument.title || `Untitled project — ${chapters[chapter]}`, chapter_number: chapter + 1, body: draft, updated_at: new Date().toISOString() }).eq("id", documentId); }
      if (documentId) await supabase.from("dreamboard_document_versions").insert({ owner_id: wmUser.id, document_id: documentId, label: snapshot.label, body: draft, word_count: words });
  ...
  const postToLounge = async () => { const body = loungeText.trim(); if (!body) return; const supabase = getSupabaseBrowserClient(); if (!supabase || !wmUser || !wmHandle) { setNotice("Set up your Passport before publishing to the shared Lounge."); setActive("Passport"); return; } const { data, error } = await supabase.from("dreamboard_lounge_posts").insert({ author_id: wmUser.id, author_label: displayName || `@${wmHandle}`, body, topic: "From Dreamboard" }).select("id, author_label, body, topic, created_at").single(); if (error || !data) { setNotice("Dreamboard could not publish that post. Please try again after confirming your Passport."); return; } setPosts(prev => [toLoungePost(data as CommunityPostRow), ...prev]); setLoungeText(""); setNotice("Your update is now shared in WOW World Lounge."); };
  ...
  const askAI = async () => {
    ...
    try {
      const response = await fetch("/api/ai", { method: "POST", headers: { "Content-Type": "application/json" }, body: JSON.stringify({ prompt: aiPrompt, context: `Project: ${writingDocument?.title || "Untitled project"}. Active chapter: ${chapters[chapter]}. Draft excerpt: ${draft.slice(0, 2200)}` }) });
  ...
  const sendWmMagicLink = async () => {
    ...
    setWmStatus("sent"); setWmMessage(`Passport sign-in email sent to ${wmEmail.trim()}. Open the link in that email, then return here. If it is not visible in a few minutes, check Spam or Promotions.`);
  };
  const saveWmProfile = async () => {
    const supabase = getSupabaseBrowserClient();
    const handle = wmHandle.trim().toLowerCase();
    if (!supabase || !wmUser) return;
    if (!/^[a-z0-9][a-z0-9_-]{2,29}$/.test(handle)) { setWmStatus("error"); setWmMessage("Choose 3–30 lowercase letters, numbers, _ or - for your Passport handle."); return; }
    setWmStatus("saving");
    ...
    if (wmError || profileError) { setWmStatus("error"); setWmMessage("Your account is signed in, but Dreamboard could not save the Passport yet. Please try once more."); return; }
    setWmStatus("saved"); setWmMessage(`Passport @${handle} is ready across WOW World.`);
  };
  const saveCreatorSettings = async () => {
    const supabase = getSupabaseBrowserClient();
    if (!supabase || !wmUser) { setNotice("Sign in with your Passport before saving creator settings."); setActive("Passport"); return; }
  ...
  const signOutWm = async () => { const supabase = getSupabaseBrowserClient(); if (!supabase) return; await supabase.auth.signOut(); setWmUser(null); setWmHandle(""); setWmStatus("ready"); setWmMessage("Signed out of Passport on this device."); };
  ...
  return <main className={`os-shell theme-${dreamTheme}`}>
    ...
    <section className="stage">
      ...
      {active === "Creative Graph" && <CreativeGraph entries={vaultEntries} nodes={graphNodes} edges={graphEdges} signedIn={Boolean(wmUser)} onVault={() => setActive("Knowledge Vault")} onWmId={() => setActive("Passport")} />}
      ...
      {active === "Projects" && <section className="view"><div className="view-heading"><span className="eyebrow">YOUR CREATIONS</span><h2>{writingDocument?.title || "Start your first project."}</h2><p>Dreamboard begins empty on purpose. Bring your real source material, then give the project a name when you are ready.</p></div><div className="project-focus"><div className="book-cover"><small>YOUR</small><strong>VISION</strong><em>Dreamboard</em></div><div><span className="eyebrow">PRIVATE PROJECT · YOURS TO DEFINE</span><h3>{wordCount ? `${wordCount.toLocaleString()} words in progress` : "No placeholder manuscript"}</h3><div className="meter"><i style={{ width: `${Math.min(100, Math.max(4, wordCount / 20))}%` }} /></div><p>{wordCount.toLocaleString()} chapter words · {notes.length} source pieces · {organized ? "themes reviewed" : "themes ready to discover"}</p><button className="gold" onClick={() => setActive("Writing Studio")}>{wordCount ? "Open Writing Studio" : "Begin writing"} <b>→</b></button></div></div></section>}
      ...
      {active === "Passport" && <Passport user={wmUser} email={wmEmail} setEmail={setWmEmail} handle={wmHandle} setHandle={setWmHandle} status={wmStatus} message={wmMessage} onSend={sendWmMagicLink} onSave={saveWmProfile} onSignOut={signOutWm} />}
...
function Passport({ user, email, setEmail, handle, setHandle, status, message, onSend, onSave, onSignOut }: { user: User | null; email: string; setEmail: (value: string) => void; handle: string; setHandle: (value: string) => void; status: string; message: string; onSend: () => void; onSave: () => void; onSignOut: () => void }) { const missingConnection = status === "needs-connection"; return <section className="view wm-id"><div className="view-heading"><span className="eyebrow">ONE PASSPORT · WOW WORLD</span><h2>Your WOW World Passport.</h2><p>Your secure Passport carries your creator profile through Dreamboard and the World of Wealth—encouraging people to live in the overflow.</p></div><div className="wm-grid"><section className="wm-card wm-orbit-card"><div className="wm-seal">WOW</div><span className="eyebrow">WORLD OF WEALTH PASSPORT</span><h3>{user ? `Welcome, ${user.email}` : "Make the work yours."}</h3><p>{user ? "Your account is verified by Supabase. Choose a Passport handle for your place in WOW World." : "Enter your email and Dreamboard sends a secure sign-in link. No additional password to remember."}</p><div className="wm-path"><span>Dreamboard</span><i>→</i><span>WOW Lounge</span><i>→</i><span>WOW Shop</span><i>→</i><span>WOW Radio</span></div></section><section className="wm-card wm-form">{missingConnection ? <><span className="eyebrow">CONNECTION REQUIRED</span><h3>Link the Supabase project.</h3><p>Passport is built into Dreamboard. Add the project URL and publishable key in Vercel, then it becomes live for every visitor.</p><div className="connection-note"><b>Safe connection values only:</b><span>Use your Project URL and Publishable key. Never use a secret key or service-role key in Dreamboard.</span></div></> : !user ? <><span className="eyebrow">CREATE OR SIGN IN</span><h3>Start with your email.</h3><label>EMAIL ADDRESS<input type="email" value={email} onChange={event => setEmail(event.target.value)} placeholder="you@example.com" /></label><button className="gold wide" onClick={onSend} disabled={status === "sending"}>{status === "sending" ? "Sending email…" : "Email my Passport link"} <b>→</b></button><p className="wm-message">{message || "We’ll send a secure Passport sign-in link to this email. Check Spam or Promotions if it does not arrive soon."}</p></> : <><span className="eyebrow">CLAIM YOUR PASSPORT</span><h3>Choose the name people will know.</h3><label>PASSPORT HANDLE<input value={handle} onChange={event => setHandle(event.target.value)} placeholder="above_the_hill" autoCapitalize="none" /></label><button className="gold wide" onClick={onSave} disabled={status === "saving"}>{status === "saving" ? "Saving Passport…" : "Save my Passport"} <b>→</b></button><p className="wm-message">{message || "Your Passport stays private until you choose to share in WOW World."}</p><button className="text-button" onClick={onSignOut}>Sign out on this device</button></>}</section></div></section>; }

function CreativeGraph({ entries, nodes, edges, signedIn, onVault, onWmId }: { entries: VaultEntry[]; nodes: GraphNode[]; edges: GraphEdge[]; signedIn: boolean; onVault: () => void; onWmId: () => void }) { const sources = nodes.filter(node => node.node_type === "source"); const themes = Array.from(new Set(entries.flatMap(entry => entry.tags || []).filter(Boolean))).slice(0, 12); return <section className="view graph-view"><div className="view-heading split"><div><span className="eyebrow">CREATIVE GRAPH · PRIVATE TO YOU</span><h2>See the connections inside the archive.</h2><p>Each saved vault entry becomes a source node. Themes are surfaced from your tags, and relationships will grow from your approved organizing decisions—not invented facts.</p></div><button className="gold" onClick={signedIn ? onVault : onWmId}>{signedIn ? "Add source material" : "Set up WM ID"} <b>→</b></button></div><div className="graph-canvas"><div className="graph-glow" /><div className="graph-center"><span>CREATIVE<br />GRAPH</span><small>{nodes.length} nodes · {edges.length} links</small></div>{sources.slice(0, 8).map((node, index) => <article key={node.id} className={`graph-node graph-node-${index % 8}`}><i>✦</i><b>{node.label}</b><small>Source</small></article>)}{themes.map((theme, index) => <span key={theme} className={`graph-theme graph-theme-${index % 6}`}>{theme}</span>)}{!nodes.length && <div className="graph-empty"><span>⌬</span><h3>Your first graph is waiting.</h3><p>Add a note, journal entry, or imported text to create a private source node.</p></div>}</div><section className="graph-truth"><div><span className="eyebrow">HOW IT WORKS TODAY</span><h3>Evidence before imagination.</h3></div><p>{signedIn ? "Dreamboard saves your source entries and their graph nodes under your signed-in account. Automatic semantic links and embeddings come after you connect an embedding provider; until then, the app shows only the material you have actually saved and tagged." : "WM ID protects your graph by making it private to your creator account. Sign in before storing real material."}</p></section></section>; }
