...
function CreativeGraph({ entries, nodes, edges, signedIn, onVault, onWmId }: { entries: VaultEntry[]; nodes: GraphNode[]; edges: GraphEdge[]; signedIn: boolean; onVault: () => void; onWmId: () => void }) { const sources = nodes.filter(node => node.node_type === "source"); const themes = Array.from(new Set(entries.flatMap(entry => entry.tags || []).filter(Boolean))).slice(0, 12); return <section className="view graph-view"><div className="view-heading split"><div><span className="eyebrow">CREATIVE GRAPH · PRIVATE TO YOU</span><h2>See the connections inside the archive.</h2><p>Each saved vault entry becomes a source node. Themes are surfaced from your tags, and relationships grow from your approved organizing decisions—not invented facts.</p></div><button className="gold" onClick={signedIn ? onVault : onWmId}>{signedIn ? "Add source material" : "Set up Passport"} <b>→</b></button></div><div className="graph-canvas"><div className="graph-glow" /><div className="graph-center"><span>CREATIVE<br />GRAPH</span><small>{nodes.length} nodes · {edges.length} links</small></div>{sources.slice(0, 8).map((node, index) => <article key={node.id} className={`graph-node graph-node-${index % 8}`}><i>✦</i><b>{node.label}</b><small>Source</small></article>)}{themes.map((theme, index) => <span key={theme} className={`graph-theme graph-theme-${index % 6}`}>{theme}</span>)}{!nodes.length && <div className="graph-empty"><span>⌬</span><h3>Your first graph is waiting.</h3><p>Add a note, journal entry, or imported text to create a private source node.</p></div>}</div><section className="graph-truth"><div><span className="eyebrow">HOW IT WORKS TODAY</span><h3>Evidence before imagination.</h3></div><p>{signedIn ? "Dreamboard saves your source entries and graph nodes under your signed-in account. Automatic semantic links and embeddings come after you connect an embedding provider; until then, the app shows only material you have actually saved and tagged." : "Your Passport protects the graph by making it private to your creator account. Sign in before storing real material."}</p></section></section>; }
