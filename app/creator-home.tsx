...
export function CreatorHome({ notes, draft, wordCount, organized, wisdomMode, creatorSeason, onGo, onOrganize, vault, snapshots, importBatches, projectTitle }: {
  ...
  return <section className="home view">
    ...
    <div className="home-grid">
      ...
      <section className="home-card vault-home"><div className="card-head"><div><span className="eyebrow">KNOWLEDGE VAULT</span><h3>{notes.length ? "Return to what matters." : "Your vault is ready."}</h3></div><button onClick={() => onGo(notes.length ? "Knowledge Vault" : "Bulk Import")}>{notes.length ? "Open vault →" : "Import material →"}</button></div>{notes.length ? notes.slice(0, 3).map(note => <button className="note-row" key={note.id} onClick={() => onGo("Knowledge Vault")}><span>✦</span><div><b>{note.title}</b><small>{note.kind} · {note.date}</small></div><em>{note.tags[0]}</em></button>) : <p className="empty-state">No example material is stored here. Import a file, save a journal entry, or begin with one real note.</p>}</section>
