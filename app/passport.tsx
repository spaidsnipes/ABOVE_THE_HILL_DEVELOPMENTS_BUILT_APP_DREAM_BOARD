...
export function PassportView({ user, email, setEmail, code, setCode, handle, setHandle, status, message, onSend, onVerifyCode, onCheckSession, onSave, onSignOut, notify, localMigration, migrationState, onMigrateLocalWork }: { user: User | null; email: string; setEmail: (value: string) => void; code: string; setCode: (value: string) => void; handle: string; setHandle: (value: string) => void; status: string; message: string; onSend: () => void; onVerifyCode: () => void; onCheckSession: () => void; onSave: () => void; onSignOut: () => void; notify: (message: string) => void; localMigration: LocalMigrationSummary; migrationState: "idle" | "migrating" | "complete" | "error"; onMigrateLocalWork: () => void }) {
  ...
  return <section className="view wm-id">
    ...
    <div className="wm-grid">
      ...
      <section className="wm-card wm-form">{missingConnection ? <><span className="eyebrow">CONNECTION REQUIRED</span><h3>Link the Supabase project.</h3><p>Passport is built into Dreamboard. Add the project URL and publishable key in Vercel, then it becomes live for every visitor.</p><div className="connection-note"><b>Safe connection values only:</b><span>Use your Project URL and Publishable key. Never use a secret key or service-role key in Dreamboard.</span></div></> : !user ? <><span className="eyebrow">CREATE OR SIGN IN</span><h3>Start with your email.</h3><label>EMAIL ADDRESS<input type="email" value={email} onChange={event => setEmail(event.target.value)} placeholder="you@example.com" /></label><button className="gold wide" onClick={onSend} disabled={status === "sending"}>{status === "sending" ? "Sending email…" : status === "sent" ? "Send a fresh sign-in email" : "Email my Passport link"} <b>→</b></button>{status === "sent" && <><div className="passport-code"><label>EMAIL CODE<input inputMode="numeric" autoComplete="one-time-code" value={code} onChange={event => setCode(event.target.value.replace(/\s/g, ""))} placeholder="Enter code from email" /></label><button className="ghost" onClick={onVerifyCode} disabled={!code.trim()}>Verify this device</button></div><button className="text-button" onClick={onCheckSession}>I opened the link here — check Passport</button></>}<p className="wm-message">{message || "We’ll send a secure Passport sign-in email. Check Spam or Promotions if it does not arrive soon."}</p><p className="wm-message"><b>Use the device you want signed in:</b> opening an email link signs in that browser. If your email arrives on your phone, enter its one-time code here instead. Code entry becomes available after Supabase’s email template includes the code.</p></> : <><span className="eyebrow">CLAIM YOUR PASSPORT</span><h3>Choose the name people will know.</h3><label>PASSPORT HANDLE<input value={handle} onChange={event => setHandle(event.target.value)} placeholder="above_the_hill" autoCapitalize="none" /></label><button className="gold wide" onClick={onSave} disabled={status === "saving"}>{status === "saving" ? "Saving Passport…" : "Save my Passport"} <b>→</b></button><p className="wm-message">{message || "Your Passport stays private until you choose to share in WOW World."}</p><button className="text-button" onClick={onSignOut}>Sign out on this device</button></>}</section>
    </div>
