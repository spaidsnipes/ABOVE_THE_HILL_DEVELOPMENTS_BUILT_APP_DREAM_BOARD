...
export function PassportView({ user, email, setEmail, handle, setHandle, status, message, onSend, onSave, onSignOut, notify }: { user: User | null; email: string; setEmail: (value: string) => void; handle: string; setHandle: (value: string) => void; status: string; message: string; onSend: () => void; onSave: () => void; onSignOut: () => void; notify: (message: string) => void }) {
  ...
  return <section className="view wm-id">
    ...
    <div className="wm-grid">
      ...
      <section className="wm-card wm-form">{missingConnection ? <><span className="eyebrow">CONNECTION REQUIRED</span><h3>Link the Supabase project.</h3><p>Passport is built into Dreamboard. Add the project URL and publishable key in Vercel, then it becomes live for every visitor.</p><div className="connection-note"><b>Safe connection values only:</b><span>Use your Project URL and Publishable key. Never use a secret key or service-role key in Dreamboard.</span></div></> : !user ? <><span className="eyebrow">CREATE OR SIGN IN</span><h3>Start with your email.</h3><label>EMAIL ADDRESS<input type="email" value={email} onChange={event => setEmail(event.target.value)} placeholder="you@example.com" /></label><button className="gold wide" onClick={onSend} disabled={status === "sending"}>{status === "sending" ? "Sending email…" : status === "sent" ? "Send a fresh sign-in link" : "Email my Passport link"} <b>→</b></button><p className="wm-message">{message || "We’ll send a secure Passport sign-in link to this email. Check Spam or Promotions if it does not arrive soon."}</p><p className="wm-message">After opening the email, return to this same Dreamboard tab. Your Passport will recognize the signed-in session automatically.</p></> : <><span className="eyebrow">CLAIM YOUR PASSPORT</span><h3>Choose the name people will know.</h3><label>PASSPORT HANDLE<input value={handle} onChange={event => setHandle(event.target.value)} placeholder="above_the_hill" autoCapitalize="none" /></label><button className="gold wide" onClick={onSave} disabled={status === "saving"}>{status === "saving" ? "Saving Passport…" : "Save my Passport"} <b>→</b></button><p className="wm-message">{message || "Your Passport stays private until you choose to share in WOW World."}</p><button className="text-button" onClick={onSignOut}>Sign out on this device</button></>}</section>
    </div>
