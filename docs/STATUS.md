...
- **Guest → Passport migration and accessibility** (2026-07-26, Milestone 29) — a signed-in creator can preview local notes, draft words, and versions before explicitly copying them into private cloud records. The local originals are never deleted; version IDs are remembered locally so a retry does not duplicate versions. Dreamboard now also resets the workspace view on navigation, has a keyboard-visible skip link, and adds high-contrast focus treatment to the core controls.
- **Archive Scale, ownership, and launch truthfulness** (2026-07-26, Milestones 30–35) — the app now plans archive intake up to 7,000 files per staged batch and processes eligible text in resumable 500-file windows; each original keeps a private batch receipt. Creators can download a current-device JSON workspace archive without deleting source material or falsely claiming cloud-only originals are included. Launch Readiness reports real Passport, private-intake, local-workspace, and WOW World connection conditions. WOW World route handoffs deliberately exclude Passport and private data until an explicit shared-auth consent flow exists.
